...
function Show-NotifyBalloon {
	...
	$Balloon.BalloonTipTitle = 'BioniDKU OSTE System Update available'
	$Balloon.Visible = $true
	$Balloon.ShowBalloonTip(7000)
}

Remove-Item -Path $env:SYSTEMDRIVE\Bionic\Hikarefresh\Hikarefreshinfo.ps1 -Force
Start-Process $env:SYSTEMDRIVE\Bionic\Hikarefresh\wget.exe -Wait -NoNewWindow -ArgumentList "https://github.com/Bionic-OSE/BioniDKU-hikaru3/releases/latest/download/Hikarefreshinfo.ps1" -WorkingDirectory "$env:SYSTEMDRIVE\Bionic\Hikarefresh"
...
elseif ($serviceremote -ne $servicer) {
	...
	Start-Process $env:SYSTEMDRIVE\Bionic\Hikarefresh\wget.exe -Wait -NoNewWindow -ArgumentList "https://github.com/Bionic-OSE/BioniDKU-hikaru3/releases/latest/download/Hikarup.7z" -WorkingDirectory "$env:SYSTEMDRIVE\Bionic"
