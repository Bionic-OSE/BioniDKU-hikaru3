...
function Show-NotifyBalloon($status) {
	switch ($status) {
		...
		1 {$title = 'BioniDKU OSXE System Update available'; $text = 'For more information, please open any BioniDKU Menus and select 9'}
	}
...
Remove-Item -Path $env:SYSTEMDRIVE\Bionic\Hikarefresh\Servicinfo.ps1 -Force
Start-Process $env:SYSTEMDRIVE\Bionic\Hikarefresh\wget.exe -Wait -NoNewWindow -ArgumentList "https://github.com/Bionic-OSE/BioniDKU-hikaru3/releases/latest/download/Servicinfo.ps1 -O Servicinfo.ps1" -WorkingDirectory "$env:SYSTEMDRIVE\Bionic\Hikarefresh"
...
elseif ($serviceremote -ne $servicer) {
	...
	Start-Process $env:SYSTEMDRIVE\Bionic\Hikarefresh\wget.exe -Wait -NoNewWindow -ArgumentList "https://github.com/Bionic-OSE/BioniDKU-hikaru3/releases/latest/download/Servicer.7z" -WorkingDirectory "$PSScriptRoot\Delivery"
...
Start-Process $env:SYSTEMDRIVE\Bionic\Hikarefresh\wget.exe -Wait -NoNewWindow -ArgumentList "https://github.com/Bionic-OSE/BioniDKU-hikaru3/releases/latest/download/VersinfOSXE.ps1 -O Versinfo.ps1" -WorkingDirectory "$env:SYSTEMDRIVE\Bionic\Hikarefresh"
